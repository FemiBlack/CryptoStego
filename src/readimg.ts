...
function prepare_read_data(data_bits: Array<number>, enc_key: string) {
...
function get_bits_lsb(imgData: ImageData): Array<number> {
  var result = Array();
...
export function readMsgFromCanvas_base(
  ...
  try {
    c = document.getElementById(canvasid) as HTMLCanvasElement;
    ctx = c?.getContext("2d");
